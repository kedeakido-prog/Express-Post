...
const SyncEngine = {
  ...
  setupRealtimeListeners() {
    ...
    // Throttled: movement ticks fire a DB UPDATE every ~15s; we don't need
    // a full SELECT * on every tick — once per 10s is sufficient for position
    // updates. Genuine edits (origin/destination/status changes) also broadcast
    // shipment_update, and the tracking.html handler calls refreshShipmentCache()
    // itself before re-rendering, so data is always fresh when displayed.
    var _shipRefreshThrottle = null;
    client.channel('epa-ships-v8')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'shipments' }, async (payload) => {
        ...
        // For INSERT/UPDATE: throttle to one refresh per 8 seconds
        if (!_shipRefreshThrottle) {
          _shipRefreshThrottle = setTimeout(async () => {
            _shipRefreshThrottle = null;
            await SyncEngine.refreshShipmentCache();
          }, 8000);
        }
        _broadcast({ type: 'shipment_update', ts: Date.now() });
